...
ALLOWED_SOURCES = {s.strip().upper() for s in os.getenv(
    "FA_NEWS_SOURCES","US_FED_PR,ECB_PR,BOE_PR,BOJ_PR,RBA_MR,US_TREASURY,JP_MOF_FX"
).split(",") if s.strip()}
...
def _html_to_text(s: str) -> str:
    """Грубое, но устойчивое извлечение текста из HTML."""
    if not s: return ""
...
def parse_fomc_calendar(html: str, url: str) -> List[CalEvent]:
    """
    Стабильный парсер FOMC: ищем все даты формата 'Sep 16–17, 2025' / 'September 16, 2025' и т.п.
    Берём первый день диапазона, время ставим 14:00Z.
    """
    ...
    for m in pat.finditer(text):
        ...
        out.append(CalEvent(dt, "united states", "USD", "FOMC Meeting / Rate Decision", "high", "FOMC", url))
    # дедуп по дате
    uniq = {}
    for ev in out:
        uniq[ev.utc.date()] = ev
...
def parse_boj_calendar(html: str, url: str) -> List[CalEvent]:
    """
    BoJ: тянем все yyyy[./-]mm[./-]dd из текста.
    """
    text = _html_to_text(html)
    out: List[CalEvent] = []
    for m in re.finditer(r"(20\d{2})[./-]\s?(\d{1,2})[./-]\s?(\d{1,2})", text):
        try:
            ...
            if not (2000 <= y <= 2100):  # страховка
                continue
            dt = datetime(y, mo, d, 3, 0, 0, tzinfo=timezone.utc)  # утро Токио ~03:00Z
            out.append(CalEvent(dt, "japan", "JPY", "BoJ Monetary Policy Meeting", "high", "BoJ", url))
        except Exception:
            continue
    # дедуп по дате
    uniq = {}
...
def collect_calendar() -> List[CalEvent]:
    ...
    # FOMC
    txt, code, f_url = fetch_text("https://www.federalreserve.gov/monetarypolicy/fomccalendars.htm")
    if code:
        ev = parse_fomc_calendar(txt, f_url)
        log.info("FOMC parsed: %d", len(ev))
        events += ev

    # BoJ — сначала индекс, затем дочерние страницы (некоторые годы вынесены в отдельные .htm)
    idx_txt, code, idx_url = fetch_text("https://www.boj.or.jp/en/mopo/mpmsche_minu/index.htm")
    if code:
        events += parse_boj_calendar(idx_txt, idx_url)
        # подхват страниц из /mpmsche_minu/..../index.htm
        for m in re.finditer(r'href="(/en/mopo/mpmsche_minu/[^"]+?\.htm)"', idx_txt, re.I):
            child = "https://www.boj.or.jp" + m.group(1)
            c_txt, c_code, c_url = fetch_text(child)
            if c_code:
                events += parse_boj_calendar(c_txt, c_url)
...
class NewsItem:
    ...
    def to_row(self) -> List[str]:
        ...
        return [_to_utc_iso(self.ts_utc), self.source, clean_title, self.url,
                self.countries, self.ccy, self.tags, self.importance_guess, self.key_hash()]

def _year_in_url(u: str) -> Optional[int]:
...
def collect_news() -> List[NewsItem]:
    ...
    min_year = now.year - 1  # отсекаем глубокий архив
    ...
    # JP MoF FX (если страница отвечает)
...
# ---------- Investor digest (короткая сводка) ----------
def write_investor_digest(sh, fa_signals: Dict[str, dict]):
    ...
    for sym in SYMBOLS:
        ...
        lines.append(f"• {sym}: {icons.get(s['risk'],'🟢')} 0% | {s['risk']}/{s['bias']} | {s.get('reason','base')}")
...
def collect_once():
    log.info("collector… sheet=%s ws=%s tz=%s window=[-%dd,+%dd]",
             SHEET_ID, CAL_WS_OUT, (LOCAL_TZ.key if LOCAL_TZ else "UTC"), FREE_LOOKBACK_DAYS, FREE_LOOKAHEAD_DAYS)
    ...
    cal_all = collect_calendar()
    log.info("CALENDAR parsed total: %d", len(cal_all))
    new_rows = []
    for ev in _calendar_window_filter(cal_all):
